...
public class CircleProgress extends View {
    ...
    protected void initVar(Context context, AttributeSet attrs) {
        ...
        maxProgress = ta.getInteger(R.styleable.CircleProgress_maxProgress, 100);
        mStrokeWidth = ta.getDimension(R.styleable.CircleProgress_progressWidth, 5);
        currProgress = ta.getInteger(R.styleable.CircleProgress_currProgress, 0);
        progressInitialPosition = ta.getInteger(R.styleable.CircleProgress_progressInitialPosition, -90);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(mPaintRectF, 0, 360, isSolid, bgPaint);
        canvas.drawArc(mPaintRectF, 0, 360*currProgress/maxProgress, isSolid, forePaint);
    }
    ...
    public void setCurrProgress(int progress) {
        if (progress < 0 ) {
            throw new IllegalArgumentException("progress must >=0 ，now progress is " + progress);
        }
    ...
    public void setMaxProgress(int maxProgress){
        if (maxProgress<=0){
            throw new IllegalArgumentException("maxProgress must >0 ,now progress is "+ maxProgress);
        }
